...
#endif
    {
        [SerializeField] AnimateHandOnInput LeftHand, RightHand;
        ...
        private void Update()
        {
            if ((RightHand.gripValue > 0 || LeftHand.gripValue > 0) && !IsTalkKeyHeld)
            {
                talkKeyInteract?.Invoke(true);
                IsTalkKeyHeld = true;
            }
            else if(IsTalkKeyHeld && !(RightHand.gripValue > 0 || LeftHand.gripValue > 0))
            {
                talkKeyInteract?.Invoke(false);
                IsTalkKeyHeld = false;
            }
